...
function processImage(imageFile) {
    ...
    if (File.exists(roiPath)) {
        ...
        // Loop through each ROI to get its name and measure
        roiCount = RoiManager.size;
        for (i = 0; i < roiCount; i++) {
            // Select the ROI
            RoiManager.select(i);
            roiName = RoiManager.getName(i);  // Get the ROI name

            // Add the ROI name as the label to the results
            roiManager("Measure");
            setResult("Label", nResults-1, roiName); // Set the ROI name as the label
        }
